...
public class BeatBox.SpaceWidget : StyledContentBox {
    ...
    public SpaceWidget (uint64 size) {
        ...
        get_style_context().add_class("spaceWidgetBase");
    ...
    public void set_size (uint64 size) {
        ...
        if (size < used_space) {
            critical("new total size is smaller than used size.\n");
            return;
        }
    ...
    public void update_item_size (int index, uint64 size) {
        ...
        if (item != null && (item.size + free_space_size) >= size) {
            ...
            critical("Couldn't update item [index = %d]. Not enough free space.\n", index);
        }
    }

    private int add_item_at_pos (string name, uint64 size, ItemColor color, ItemPosition pos) {
        if (size > free_space_size) {
            critical("Couldn't add '%s' item. Not enough free space.\n", name);
            return -1; // ERROR
        }
    ...
    private void update_bar_item_sizes () {
        ...
        status_label.set_text(_("Using %s of %s (%.0f%)").printf(GLib.format_size(used), GLib.format_size(total_size), p));
    }
...
private class BeatBox.SpaceWidgetBarItem : Gtk.Button {
...
private class BeatBox.SpaceWidgetBarFullItem : SpaceWidgetBarItem {
...
private class BeatBox.LegendItem : Gtk.Button {
    public SpaceWidget.ItemColor color;
