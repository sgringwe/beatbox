	...
	public static Store.Release? parseRelease(Xml.Node* node) {
		...
		for (Xml.Node* iter = node->children; iter != null; iter = iter->next) {
			...
			if(name == "title")
				...
				tm.strptime(content.substring(0, content.index_of("T", 0)), "%Y-%m-%d");
				...
				for(Xml.Node* format = iter->children; format != null; format = format->next) {
					var formatO = parseFormat(format);
					
					if(formatO != null)
						rv.formats.add(formatO);
				}
			}
			else if(name == "label") {
	...
	public static Store.Track? parseTrack(Xml.Node* node) {
		...
		for (Xml.Node* iter = node->children; iter != null; iter = iter->next) {
			...
			if(name == "title")
				...
				rv.store_artist = parseArtist(iter);
				rv.artist = rv.store_artist.name;
			}
			else if(name == "trackNumber")
				rv.track = int.parse(content);
			else if(name == "duration")
				rv.length = int.parse(content);
			else if(name == "explicitContent")
				rv.explicitContent = (content == "true") ? true : false;
			else if(name == "isrc")
				rv.isrc = content;
			else if(name == "release")
				rv.store_release = parseRelease(iter);
			else if(name == "url")
				rv.url = content;
			else if(name == "price")
				rv.price = parsePrice(iter);
		...
		message("parsed %s %s %d %d %f", rv.title, rv.artist, (int)rv.track, (int)rv.length, rv.price.val);
...
//		message("2");
		Store.Format rv = new Store.Format(0);
		...
		for (Xml.Node* iter = node->children; iter != null; iter = iter->next) {
			...
			if(name == "fileFormat")
				...
				rv.drmFree = (content.down() == "true") ? true : false;
		}
