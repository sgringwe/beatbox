...
public class Store.SearchList : Gtk.ScrolledWindow {
	...
	public void addItem(GLib.Object obj) {
		...
		if(obj is Store.Track) {
			...
			store.set(iter, 0, track, 1, store.iter_n_children(null).to_string(), 2, track.title, 3, track.store_artist.name, 4, BeatBox.TimeUtils.pretty_time_mins(track.length), 5, track.price.formattedPrice);
		}
	 ...
	 void resized(Allocation rectangle) {
		if(alreadyResized) {
